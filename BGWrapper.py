...
class Characteristic(object):
    ...
    def onNotify(self, new_value):
        ...
        self.unpack()
        if self.notify_cb:
            self.notify_cb(self.handle, self.byte_value)
...
def disconnect(conn):
    ble.send_command(ser, ble.ble_cmd_connection_disconnect(conn))
    ble.check_activity(ser)
...
def writecommand(conn, handle, value):
    if(handle==0):
        print "Invalid handle!  Did you forget a call to Peripheral.replaceCharacteristic(c)?"
    ble.send_command(ser, ble.ble_cmd_attclient_write_command(conn,handle,value))
    ble.check_activity(ser)
